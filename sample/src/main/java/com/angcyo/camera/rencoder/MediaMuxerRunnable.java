package com.angcyo.camera.rencoder;
...
import com.angcyo.camera.CameraWrapper;
import com.angcyo.camera.FileSwapHelper;
...
public class MediaMuxerRunnable extends Thread {
    ...
    private boolean initMuxer() {
        ...
        fullPath.append(FileSwapHelper.BASE_VIDEO);
        FileSwapHelper.cleanFolder(fullPath.toString(), FileUtils.T_FLASH_PATH);
        if (FileSwapHelper.checkCardSpace(FileUtils.T_FLASH_PATH)) {
            return false;
        }
...
//            restart();
    ...
    private void readyStart(String filePath, boolean restart) throws IOException {
        ...
        if (restart) {
            addTrackIndex(TRACK_AUDIO, audioMediaFormat);
        } else {
            isAudioAdd = false;
            if (audioThread != null) {
...
//                ||
        if ((index == TRACK_VIDEO && isVideoAdd())) {
            restart();
            return;
        }
...
//            if (DEBUG)
    ...
    public void run() {
        if (!initMuxer()) {
            if (DEBUG) Log.e("angcyo-->", "混合器没有达到启动条件!");
            exitThread();
            return;
        }

        isThreadStart = true;
        while (!isExit && FileUtils.isTFlashCardExists()) {
            if (isMuxerStart()) {
